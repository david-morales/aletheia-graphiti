...
async def resolve_extracted_nodes(
    ...
    for idx, (node, candidates) in enumerate(
        ...
        # fork 533af79: identifier-name entity types dedup by exact normalized
        # name only — no fuzzy, no LLM. Load-bearing for deterministic dedup of
        # types carrying a unique identifier (e.g. AGE narrative persons by DNI).
        if _is_identifier_name_type(node, entity_types):
            id_state = DedupResolutionState(
                resolved_nodes=[None], uuid_map={}, unresolved_indices=[]
            )
            _resolve_exact_only([node], indexes, id_state)
            if id_state.resolved_nodes[0] is not None:
                _commit_resolution(
                    state,
                    id_state.resolved_nodes[0],
                    id_state.uuid_map,
                    id_state.duplicate_pairs,
                    idx,
                )
            continue
