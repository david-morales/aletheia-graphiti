...
async def extract_edges(
    ...
    # Resolve LLM-returned entity names against the node list, tolerating case
    # differences and partial (containment) matches so valid edges are not
    # silently dropped when the LLM does not echo names verbatim (fork 64898ee).
    def _resolve_entity_name(name: str) -> str | None:
        ...
        for canonical in name_to_node:
            if canonical.lower() == name_lower:
                return canonical

        # 3. Containment — prefer the longest overlapping canonical name
        best: str | None = None
        for canonical in name_to_node:
    ...
    # Validate entity names, recovering non-exact LLM names via _resolve_entity_name
    ...
    for edge_data in all_edges_data:
        ...
        # Resolve LLM-returned names against the nodes list (fork 64898ee/53597d5:
        # tolerate case/containment so valid edges are not silently dropped)
        resolved_source = _resolve_entity_name(source_name)
        ...
        resolved_target = _resolve_entity_name(target_name)
        ...
        source_node = name_to_node[resolved_source]
        target_node = name_to_node[resolved_target]
...
async def resolve_extracted_edges(
    ...
    phantom_indices = [
        ...
        if e.source_node_uuid not in uuid_entity_map
        or e.target_node_uuid not in uuid_entity_map
    ]
    if phantom_indices:
        logger.warning(
        ...
        valid_indices = [
            i for i in range(len(extracted_edges)) if i not in set(phantom_indices)
        ]
