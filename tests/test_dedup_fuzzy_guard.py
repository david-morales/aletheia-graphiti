...
Validates that the Jaccard threshold and edit-distance guard prevent
...
class TestSequentialIdentifiersNotAutoMerged:
    ...
    These have high Jaccard similarity (0.90-0.94) due to long shared prefixes
    but represent completely different entities.
    """

    @pytest.mark.parametrize(
        'name_a, name_b',
        [
            # Type-prefixed sequential IDs (the original bug)
...
class TestGenuineDuplicatesStillAutoMerge:
    """Names that ARE the same entity (typos, formatting) should still auto-merge."""
...
class TestEditDistanceGuard:
    ...
    def test_high_jaccard_high_edit_distance_defers_to_llm(self):
        ...
        This requires very long names where 3+ scattered changes still yield high Jaccard.
        """
        # Construct names long enough that 3 changes still produce Jaccard >= 0.95
        # 100-char base with 3 changes
