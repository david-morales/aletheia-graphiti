...
def _make_clients(driver=None):
    """Build a minimal GraphitiClients mock."""
    clients = MagicMock()
    clients.driver = driver or MagicMock()
...
def _explore_config():
    """SearchConfig matching explore_node's config — BFS + bm25 + cosine with node_distance reranker."""
    return SearchConfig(
        edge_config=EdgeSearchConfig(
            search_methods=[
...
async def test_empty_query_with_bfs_origins_runs_bfs():
    ...
    with (
        ...
        patch('graphiti_core.search.search.node_summary_similarity_search', new_callable=AsyncMock) as mock_node_sum,
        ...
        mock_node_ft.return_value = []
        mock_edge_ft.return_value = []
        mock_node_sim.return_value = []
        mock_node_sum.return_value = []
        mock_edge_sim.return_value = []
        mock_ep_ft.return_value = []
        mock_comm_ft.return_value = []
        mock_comm_sim.return_value = []
...
async def test_empty_query_with_bfs_skips_embedding_call():
    ...
    with (
        ...
        patch('graphiti_core.search.search.node_summary_similarity_search', new_callable=AsyncMock, return_value=[]),
